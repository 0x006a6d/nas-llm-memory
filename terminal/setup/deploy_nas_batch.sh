...
mkdir -p "$DEST"
cp "$SRC"/nightly.sh "$SRC"/run_batch.sh "$SRC"/nightly.py "$SRC"/ringi.py "$SRC"/backup.sh "$SRC"/purge.py "$SRC"/compact.py "$SRC"/edges_backfill.py "$SRC"/skill_scout.py "$DEST/"
