...
- スキル       — /context に出る構成要素の一覧を出所別に表示:
                 スキル(user / claude-config / 各プロジェクト .claude/skills / プラグイン)、
                 コマンド(commands/*.md)、エージェント(agents/*.md)、
                 Claude Code 内蔵(builtin-context.json の手動スナップショット。バイナリ
                 埋め込みで列挙不可のため、claude のバージョンが変わると要更新の注意を出す)。
                 各項目に編集可否を明示(プラグイン配布物・内蔵は編集不可)
