...
- 後閲 — バッチは無人で施行まで進み (代決)、人間は dashboard の決裁・後閲タブで事後確認する。後閲印または差し戻し。skill の施行 (skills/ 本体への登載 = 全端末配布) だけは人間の後閲印を施行条件とする (`ringi.skill_auto_execute` で解除可)
...
適用は 2 段階: コードを deploy しても `enabled` が立つまで挙動は変わらないので、先に配置して従来動作を確認してから config を切り替える。専決規程 (roles と主要フラグ) は dashboard の決裁・後閲タブからも変更できる (翌晩から反映)。
...
- 管理簿 — `record_files`。dashboard の「管理簿」タブが分類・名称・保存期間・満了する日・措置・件数・保存場所・状態を表示する (閲覧のみ。操作は夜間バッチと決裁・後閲タブ)
